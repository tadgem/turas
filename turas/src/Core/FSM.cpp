...
turas::FSM::State::State(int state, Function<int()> action, Function<void()> entry,
                           Function<void()> exit) : m_State(state), m_Action(action) {
    ZoneScoped;
    HasEntry = false;
    HasExit = false;
...
turas::FSM::FSM() : p_CurrentState(UINT16_MAX), p_RunEntry(false) {
    ZoneScoped;
}
